...
main_keyboard.add(sign_up_button)
...
async def sign_up(ms):
    await ms.answer("Введите имя пользователя (только латинский алфавит):")
    await RegistrationState.username.set()


@dispatcher.message_handler(state=RegistrationState.username)
async def set_username(message, state):
    if (crud_functions.check_user_in_database(message.text)):
...
async def set_age(message, state):
    ...
    crud_functions.add_user(data['email'], data['age'], data['username'])
